...
const METADATA_ABS_PANEL: SeriesMetadata = SeriesMetadata {
  name: TXT_ABS_PANEL,
  plot_colour: RGB_DARK_BLUE,
};
const METADATA_ABS_BACKING: SeriesMetadata = SeriesMetadata {
...
fn draw_title_and_key(
  ...
  // Add chart title
  ctx
    .fill_text(&title, LEFT_MARGIN_INSET, TOP_MARGIN_INSET)
    .unwrap();
  ...
  let longest_key_text = series_list.iter().fold(0.0, |acc: f64, s| {
    acc.max(ctx.measure_text(s.name).unwrap().width())
  });
  ...
  trace(&format!(
    "Key table contains {} columns and {} rows",
  ...
  for row_idx in 0..key_rows {
    for col_idx in 0..key_columns {
      ...
      if series_idx < series_list.len() {
        ...
        ctx
          .fill_text(
            series_list[series_idx].name,
            x + 40.0,
            y + (key_font.font_size / 2.0) - 3.0,
          )
          .unwrap();
...
fn draw_axis(canvas: &web_sys::HtmlCanvasElement, axis_info: &Axis) -> f64 {
  ...
  let (mid_height, mid_width, bottom_margin_pos, _, _) =
    canvas_dimensions(&canvas, &axis_info.start_point.x);
  ...
  ctx
    .translate(axis_info.start_point.x, axis_info.start_point.y)
    .unwrap();
  ...
  for val in axis_info.values.iter() {
    ...
    match axis_info.orientation {
      AxisOrientation::Vertical => {
        ctx
          .fill_text(&tick_label, -label_offset, TICK_LABEL_GAP)
          .unwrap();
      ...
      AxisOrientation::Horizontal => {
        ctx
          .fill_text(
            &tick_label,
            -label_offset,
            axis_info.label_font.font_size / 2.0,
          )
          .unwrap();
        ctx.translate(0.0, tick_interval).unwrap();
      }
...
fn draw_device_diagram(
  ...
  trace(&format!(
    "Available space for diagram = {} px",
    available_pxls
  ));
  ...
  trace(&format!(
    "Drawing wall at location ({},{})",
    wall_pos_x, wall_pos_y
  ));
...
fn draw_splines(
  ...
  for idx in 0..abs_points.len() - 2 {
    cps.append(&mut gen_control_points(
      &abs_points[idx],
      &abs_points[idx + 1],
      &abs_points[idx + 2],
      tension,
