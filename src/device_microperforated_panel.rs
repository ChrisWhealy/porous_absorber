...
pub fn do_microperforated_panel_device(wasm_arg_obj : JsValue) -> JsValue {
  ...
  let panel_cfg = MicroperforatedPanelConfig::new(panel_thickness_mm, repeat_distance_mm, hole_radius_mm, porosity).unwrap_or_else(|err: MicroperforatedPanelError| {
    error_msgs.push(String::from(err.description()));
  ...
  let return_value = if error_msgs.len() == 0 {
    let absorber_info = calc_engine::calculate_microperforated_panel(&air_cfg, &cavity_cfg, &display_cfg, &panel_cfg, &sound_cfg);
