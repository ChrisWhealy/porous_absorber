...
// Another name for the Identity function. See http://dkeenan.com/Lambda/ for details
const idiot = val => val

// Set an object property value and returns the updated object.  Needed for use in IIFE's
...
// values
const invertPlotData = plotData =>
  plotData.reduce(
    (acc, seriesData) => {
      seriesData.plot_points.map(
        plotPoint => {
          let yVals = acc[plotPoint.x]
          let yInfo = removeX(plotPoint)
          setProperty(acc, plotPoint.x, yVals ? yVals.concat(yInfo) : [yInfo])
        }
      )
      return acc
    },
...
const isInsideRect = (top_left_x, top_left_y, bottom_right_x, bottom_right_y) =>
  mousePos =>
