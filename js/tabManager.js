...
import { showUnits, showAndConvertUnits } from "./unitConversion.js"
import { canvasMouseOverHandler, CANVAS_CONTAINER, GRAPH_OVERLAY } from "./canvas.js"
import { isArray, isNotNullOrUndef, invertPlotData, idiot, setProperty } from "./utils.js"
...
const fetchTabFn = tabName => {
  traceInfo("fetchTabFn")(`Fetching tab "${tabName}"`)
...
// *********************************************************************************************************************
const toggleLockAbsDimsFn = tabName => {
  let labelValue = ""

  if ($id("lock_abs_dims").checked) {
    labelValue = `${$id("air_gap_mm").valueAsNumber + $id("absorber_thickness_mm").valueAsNumber} mm`
    window.lockAbsDims[tabName] = $id("air_gap_mm").valueAsNumber + $id("absorber_thickness_mm").valueAsNumber
  } else {
    window.lockAbsDims[tabName] = null
  }

  showUnits(labelValue, { id: "lock_abs_dims", units: "each" })
}

// ***************************************************************************
const balanceSliderValuesFn = (s1, s2, sum) => {
  if (s2.valueAsNumber >= sum - s1.min) {
    s2.valueAsNumber = sum - s1.min
  } else if (s2.valueAsNumber <= sum - s1.max) {
    s2.valueAsNumber = sum - s1.max
  }

  s1.valueAsNumber = sum - s2.valueAsNumber
}

// ***************************************************************************
// This function must be called every time an input value is changed
const updateScreenFn = (tabName, rangeName) => {
  // If the absorber dimensions are locked, ensure that the sum of the slider values remains fixed
  let lockedDim = window.lockAbsDims[tabName]

  if (lockedDim && lockedDim > 0) {
    let airGapSlider = $id("air_gap_mm")
    let absThknsSlider = $id("absorber_thickness_mm")

    if (rangeName === "air_gap_mm") {
      balanceSliderValuesFn(absThknsSlider, airGapSlider, lockedDim)
    }
    else if (rangeName === "absorber_thickness_mm") {
      balanceSliderValuesFn(airGapSlider, absThknsSlider, lockedDim)
    }
  }
  ...
  // The air pressure and temperature values on the configuration tab are common to all calculations and therefore must
  // always be merged into the argument object passed to WASM
...
const updateScreenAndMouseHandlerFn = tabName =>
  (wasm_response => {
...
const limitMax = (srcEl, targetEl, upperLimit, modifierFn) => {
  let modFn = modifierFn || idiot
  $id(targetEl).max = Math.min(modFn(srcEl.value), upperLimit)
}
...
const toggleLockAbsDims = traceFnBoundary("toggleLockAbsDims", toggleLockAbsDimsFn)
