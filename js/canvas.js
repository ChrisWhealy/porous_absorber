...
// *********************************************************************************************************************
const drawCrossHairs = (ctx, mousePos, chartBox) => {
  ctx.beginPath()
...
// *********************************************************************************************************************
// Draw horizontal and vertical lines on the canvas within the chart boundary and create information popup when the
// mouse pointer hovers over an absorption plot point.
//
// To acheive the correct alignment, the canvas overlay element containing these cross hairs must sit exactly on top of
// the canvas showing the graph.
//
// The returned function becomes the onmouseover event handler for the canvas overlay element
const canvasMouseOverHandler = (canvas, chartBox, seriesData) => {
    ...
    Object
      ...
      .map(
        xValStr => {
          ...
          if (isBetween(xVal + PLOT_POINT_RADIUS, xVal - PLOT_POINT_RADIUS, mousePos.x)) {
            ...
            seriesData[xValStr].map(
              plotPoint => {
                ...
                if (isBetween(yVal + PLOT_POINT_RADIUS, yVal - PLOT_POINT_RADIUS, mousePos.y))
